...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let port: u16 = from_env("PORT", 3000)?;
    ...
    // Loopback by default so a locally run mock is not exposed to the network. The Docker
    ...
    println!(
        "mock-payment-service listening on http://{}",
        listener.local_addr()?
    );

    axum::serve(listener, mock_payment_service::app()).await?;
    Ok(())
}
